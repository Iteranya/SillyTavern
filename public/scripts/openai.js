...
function setOpenAIMessages(chat) {
    ...
    for (let i = chat.length - 1; i >= 0; i--) {
        ...
        const signature = chat[j]?.extra?.reasoning_signature || null;
        const reasoning = chat[j]?.extra?.reasoning || '';
...
async function populateChatHistory(messages, prompts, chatCompletion, type = null, cyclePrompt = null) {
    ...
    for (let index = 0; index < chatPool.length; index++) {
        ...
        const chatMessage = await Message.fromPromptAsync(promptManager.preparePrompt(prompt));
        if (chatPrompt.reasoning) chatMessage.reasoning = chatPrompt.reasoning;
        ...
        if (canUseTools && Array.isArray(chatPrompt.invocations)) {
            ...
            // HARDCODED: Force includeToolReasoning to TRUE
            await toolCallMessage.setToolCalls(invocations, includeSignature, true);
            
            // HARDCODED: Brute-force staple the thought back onto the tool call just in case
